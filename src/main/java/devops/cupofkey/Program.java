...
public class Program
{
    ...
    {  
    	...
    	MasterDispatcher masterServer = null;
		try {
			masterServer = MasterServer.getDefaultMasterServer();
			masterServer.start();
		} catch (IOException e1) {
			System.out.println("serveur principal non lancé, impossible de continuer \n\n");
		}
    	...
    	if(masterServer != null){
	        try
	        {
	        	...
	        	build.append("get, set, exist, incr, mult, clear, delete, push, quit \n"); 
	        	
	        	System.out.println(build.toString());
	        	
	        	String cmd; 
	        	while(!(cmd = scan.nextLine()).equals("quit")){
				...
				System.err.println("Le programme va maintenant s'arreter");
				masterServer.interrupt();
				masterServer.join(200);
	        	secondSecondaryServer.interrupt();
	        	secondSecondaryServer.join(200);
	        	firstSecondaryServer.interrupt();
	        	firstSecondaryServer.join(200);
	        	System.exit(1);
			}
	    	masterServer.interrupt();
			masterServer.join(200);
